...
public class DishCrudOperation implements CrudOperations<Dish> {
                ...
                try(ResultSet resultSet = statement.executeQuery()){
                    while(resultSet.next()){
                        Dish dish = new Dish(
                                ...
                                getIngredientWithQuantity(resultSet.getString("id"))
                        );
                        dishes.add(dish);
                    }
                }
    ...
    public ResponseEntity putIngrdientQuantity(String dishId, List<IngredientWithQuantity> ingredientsWithQuantity) {
        for(IngredientWithQuantity ingredientWithQuantity : ingredientsWithQuantity){
            String sql = "INSERT INTO dish_ingredient (id_dish, id_ingredient, required_quantity, unity) VALUES (?,?, ?, ?)";
            try(Connection connection = dataSource.getConnection();
                PreparedStatement statement = connection.prepareStatement(sql)){
                statement.setString(1, dishId);
                statement.setString(2, ingredientWithQuantity.getIngredient().getId());
                statement.setDouble(3, ingredientWithQuantity.getRequiredQuantity());
                statement.setString(4, ingredientWithQuantity.getUnity().toString());
