...
public class IngredientCrudOperation implements CrudOperations<Ingredient> {
    ...
    public List<Ingredient> getAll() {
            List<Ingredient> ingredients = new ArrayList<>();
        String sql = "SELECT id, name, unity FROM ingredient ORDER BY id ASC ";
            try(Connection connection = dataSource.getConnection();
                PreparedStatement statement = connection.prepareStatement(sql);){
                try(ResultSet resultSet = statement.executeQuery();) {
                    while(resultSet.next()){
    ...
    public List<IngredientPrice> getAllIngredientPrice(String ingredient_id){
        List<IngredientPrice> ingredientPrices = new ArrayList<>();
        String sql = "SELECT unit_price, date FROM price WHERE id_ingredient = ? ORDER BY ABS(EXTRACT(EPOCH FROM ?::timestamp) - EXTRACT(EPOCH FROM date));";
        try(Connection connection = dataSource.getConnection();
            PreparedStatement statement = connection.prepareStatement(sql)){
            statement.setString(1, ingredient_id);
            statement.setTimestamp(2, Timestamp.valueOf(LocalDateTime.now()));
            try(ResultSet resultSet = statement.executeQuery();){
                while (resultSet.next()){
                    ingredientPrices.add(new IngredientPrice(
                            resultSet.getDouble("unit_price"),
                            resultSet.getTimestamp("date").toLocalDateTime()
                    ));
                }
    ...
    public List<Stock> getAvailableStocks(String ingredient_id){
        ...
        try(Connection connection = dataSource.getConnection();
            ...
            try(ResultSet resultSet = statement.executeQuery();){
                while(resultSet.next()){
                    Ingredient ingredient = findById(ingredient_id);
    ...
    public ResponseEntity addPrice(String ingredientId, List<CreatePrice> prices){
        for(CreatePrice price : prices){
            String sql = "INSERT INTO price (id, unit_price,id_ingredient) VALUES(?,?);";
            try(Connection connection = dataSource.getConnection();
                PreparedStatement statement = connection.prepareStatement(sql)){
                statement.setString(1, price.getId());
                statement.setDouble(2, price.getQuantity());
    ...
    public ResponseEntity addStock(String ingredientId, List<CreateStock> stocks){
        for(CreateStock stock : stocks){
            String sql = "INSET INTO stock (mouvement, quantity,unity, ingredient_id) VALUES (?,?,?,?);";
            try(Connection connection = dataSource.getConnection();
            PreparedStatement statement = connection.prepareStatement(sql)){
                statement.setString(1, stock.getId());
                statement.setDouble(2, stock.getQuantity());
                statement.setString(3, stock.getUnity().toString());
                statement.setString(4, ingredientId);
                statement.executeUpdate();
            } catch (SQLException e) {
                throw new RuntimeException(e);
            }
